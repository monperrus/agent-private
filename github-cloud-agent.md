...
API version used throughout: `2026-03-10`.
...
```bash
curl https://api.github.com/orgs/ORG/copilot/coding-agent/permissions/repositories
```
...
```bash
curl https://api.github.com/enterprises/ENTERPRISE/copilot/custom-agents
```
...
### The `agent_assignment` field on issues

The `agent_assignment` JSON field in `POST /repos/{owner}/{repo}/issues` is not listed in the standard issues API schema. It is the actual mechanism that configures the agent's run — without it, assigning `copilot-swe-agent[bot]` alone may not start the agent correctly. Known fields:
